...
def annotate_source(
    ...
    for row in rows:
        ...
        matched=(row_path.resolve()== source_path_resolved)
        if not matched and str(row_path).startswith(container_repo_root+"/"):
            relative = str (row_path)[len(container_repo_root)+1:]
            matched= source_path_resolved.parts[-len(Path(relative).parts):]== Path(relative).parts
    ...
    original_lines = source_path_resolved.read_text(encoding="utf-8").splitlines()
    annotated: list[str] = []
